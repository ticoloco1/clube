...
const NO_STORE = { 'Cache-Control': 'private, no-store, max-age=0', Pragma: 'no-cache' } as const;
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!slug) {
      return NextResponse.json({ error: 'slug obrigatório' }, { status: 400, headers: NO_STORE });
    }
