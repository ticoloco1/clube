...
import { openAiTtsMp3 } from '@/lib/openAiTts';
import {
  getLivelySiteForApi,
  getServiceDb,
  getViewerUserId,
...
/**
 * POST { slug, text } → audio/mpeg (OpenAI TTS) ou JSON { clientTTS: true, text } se sem OPENAI_API_KEY.
 * Visemes são derivados no cliente a partir do texto + duração do áudio decodificado.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    if (!rateLimitLively(`tts:${ip}:${slug}`)) {
    ...
    const buf = await openAiTtsMp3(text);
    if (!buf) {
      return NextResponse.json({ clientTTS: true as const, text });
    }
    ...
    console.error('[lively-avatar/tts]', e);
